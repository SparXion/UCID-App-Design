...
export class StudentProfileService {
  async submitQuiz(studentId: string, dto: QuizSubmissionDto) {
    ...
    // Check if student exists (students must be created through signup)
    const existingStudent = await prisma.student.findUnique({
      where: { id: studentId }
    });

    if (!existingStudent) {
      throw new Error('Student not found. Please sign up first.');
    }

    // Update existing student with new quiz data
